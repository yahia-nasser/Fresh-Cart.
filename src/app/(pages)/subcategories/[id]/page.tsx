...
import getSingleSubCategory from "@/apis/getSingleSubCategory";
...
import { Products, Subcategory } from "@/app/types/product.type";
import {
  Card,
  CardContent,
  CardFooter,
...
const SingleSubCategory = async ({ params }: { params: { id: string } }) => {
  const { id } = await params;

  const data: Subcategory = await getSingleSubCategory(id);
  const products: Products[] = await getAllProducts();

  const singleSubProducts = products.filter((product) =>
    product?.subcategory?.some((sub) => sub.name === data.name)
  );

  if (singleSubProducts.length == 0) {
    return (
  ...
  return (
    <section className="px-5 md:px-0 my-10 w-full md:w-[80%] mx-auto ">
      ...
      <div className="flex flex-wrap justify-center ">
        {singleSubProducts.map((product, idx: number) => {
          return (
            <div
              key={idx}
